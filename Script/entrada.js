...
async function enviar(event) {
  ...
  try {
    ...
    const response = await fetch("http://ptcc.elementfx.com/digital.php", {
      method: "POST",
      credentials: "include",
...
window.onload = async function () {
  try {
    const response = await fetch("http://ptcc.elementfx.com/acesso.php", {
      method: "POST",
      credentials: "include",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
    });
    ...
    if (response.ok) {
      ...
      if (texto && Array.isArray(info.data)) {
        ...
        info.data.forEach(function(dados) {
          const imgPath = `http://ptcc.elementfx.com/${dados.imagem}`; // ou .png dependendo do formato
